...
The contact page sets a Content-Security-Policy meta tag in **production** when Turnstile is enabled (dev skips it so Astro’s dev toolbar can run). `script-src` allows only same-origin files and Cloudflare Turnstile — contact form logic is bundled under `/_astro/` (not inline). See [`BaseLayout.astro`](../src/layouts/BaseLayout.astro).
